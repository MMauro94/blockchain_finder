...
def write_last_block(last_block):
     with open('last_block_leveldb.txt', 'w') as f:
...
if os.path.exists('last_block_leveldb.txt'):
	with open('last_block_leveldb.txt', 'r') as f:
...
db = plyvel.DB('tx_to_block/', create_if_missing=True)
blockchain = Blockchain(os.path.expanduser('./datas'))
total_tx=0
tm=time.time()
tx_in_time_slot = 0
tx_per_sec = 0
for block in blockchain.get_ordered_blocks('./datas/index', start=last_block+1):
